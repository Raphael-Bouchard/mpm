...
class Factory {
        ...
        public:
          ...
          std::shared_ptr<Tbaseclass> create(const std::string& key, Targs&&... args)
          {
            if (!this->check(key))
            {
              throw std::runtime_error("Invalid key: " + key +", not found in the factory register!");
            }
...
class Register {
 public:
  ...
  explicit Register(const std::string& key)
  {
    ...
    Factory<Tbaseclass, Targs...>::instance()->template register_factory<Tderivedclass>(key);
  }
};
